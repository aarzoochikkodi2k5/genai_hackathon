...
if "Cricket" in mode:
    ...
    with st.expander("📋 CSV Format Guide"):
        ...
        **Required columns for CSV upload:**
        ...
        - `over`: Over number (e.g., 1-20)
        - `ball`: Ball number within over (1-6)
        - `batsman`: Batsman name
        - `bowler`: Bowler name
        - `runs`: Runs scored (0-6)
        - `event`: Event type (wicket, six, boundary, four, dot)
        
        Each row represents one ball delivered.
        """)
    ...
    if uploaded_json:
        try:
            ...
            st.error(f"❌ JSON error: {str(ex)[:100]}")
    elif uploaded_csv:
        try:
            csv_content = uploaded_csv.read().decode("utf-8")
            # Validate CSV has required columns
            df_check = pd.read_csv(io.StringIO(csv_content))
            required_cols = {"batting_team", "bowling_team", "over", "ball", "batsman", "bowler", "runs", "event"}
            missing_cols = required_cols - set(c.strip().lower() for c in df_check.columns)
            
            if missing_cols:
                st.error(f"❌ CSV missing required columns: {', '.join(sorted(missing_cols))}")
            else:
                match_ctx = load_match_data(csv_content, sport="cricket")
                st.success("✅ Custom CSV loaded")
        except UnicodeDecodeError:
            st.error("❌ CSV encoding error - Please save as UTF-8")
        except pd.errors.ParserError as e:
            st.error(f"❌ CSV parsing error - Invalid CSV format: {str(e)[:80]}")
        except Exception as ex:
            st.error(f"❌ CSV error: {str(ex)[:150]}")
    else:
        try:
